...
def classify_and_plot_grid(cities, k=1):
    ...
    for (longitude, latitude), language in cities:
        predictedLanguage = knn_classify(k, cities, (longitude, latitude))
        plots[predictedLanguage][0].append(longitude)
        plots[predictedLanguage][1].append(latitude)
    ...
    plt.legend(loc=0)          # let matplotlib choose the location
    plt.axis([-130,-60,20,55]) # set the axes
    plt.title("Predicted Programming Languages")
    plt.show()
